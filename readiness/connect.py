"""Native cognis-connect emit for readiness — forward findings to any platform.

Maps readiness's JSON output to the canonical `Finding` and forwards it via
...
Usage:
    readiness ... --format json | readiness-emit --to stix
    readiness-emit --to slack --url $WEBHOOK --dry-run < findings.json
"""
...
def map_record(rec: dict) -> dict:
    ...
    try:
        out = dict(rec)
        out.pop('unit', None)
        out.pop('composite_pct', None)
        out.pop('limiting_areas', None)
        out.pop('areas', None)
        out.pop('gaps', None)
        out['title'] = rec.get('unit', '') + ' Readiness Gap'
        out['severity'] = 'info'
        out['type'] = 'readiness'
        out['description'] = 'Readiness gap identified in unit'
        out['tags'] = ['readiness', 'gap']
        out['ipv4'] = ''
        out['domain'] = ''
        out['url'] = ''
        out['sha256'] = ''
